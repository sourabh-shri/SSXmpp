                                                    ...
                                                    managedObjectContext:(NSManagedObjectContext *)moc
{
    ...
    NSString *predicateFrmt = @"bareJidStr == %@ AND messageId == %@ AND streamBareJidStr == %@";
    NSPredicate *predicate = [NSPredicate predicateWithFormat:predicateFrmt,
	                             [messageJid bare], messageid, [streamJid bare]];
...
- (void)archiveMessage:(XMPPMessage *)message outgoing:(BOOL)isOutgoing xmppStream:(XMPPStream *)xmppStream
{
    ...
    BOOL shouldDeletePausedMessage = NO;

    NSString *messageStatus = @"Wait";
    BOOL shouldUpdateMessageStatus = NO;

    
    isComposing = [message hasComposingChatState];
    isPaused = [message hasPausedChatState];
	...
	[self scheduleBlock:^{
        ...
        if(shouldUpdateMessageStatus)
        {
            NSError *parseError = nil;
            NSDictionary *xmlDictionary = [XMLReader dictionaryForXMLString:[message XMLString] error:&parseError];
            
            archivedMessage =
            [self getMessageWithMessageId:messageJid
                                streamJid:myJid
                                messageId:[[[xmlDictionary valueForKey:@"message"] valueForKey:@"received"] valueForKey:@"id"]
                     managedObjectContext:moc];
            
            archivedMessage.recieptStatus = messageStatus;
            [archivedMessage didUpdateObject];       // Override hook
            [self didUpdateMessage:archivedMessage];
        ...
        else
        {
            ...
            else
            {
                ...
                if ([messageBody length] != 0)
                    archivedMessage.messageId = [[message attributesAsDictionary]valueForKey:@"id"];
                ...
                archivedMessage.isPaused = isPaused;
